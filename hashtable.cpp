...
hashTable::hashTable() {
    table.resize(10); //starts with 10 buckets
}

//person hashTable::search(int ID_) { concept for search functionn, not used
...
void hashTable::insert(person insertee) { 
    ...
    table[getKey(insertee.getState(), table.size())].push_back(insertee); //inserts obj
    double num = table.size();
    if (count / num > 0.75) { // rehashes if the load factor > 0.75
...
//void hashTable::del() { //unused delete function
...
{
    int sum = 0; //key=sum of (ascii values-31)
...
void hashTable::sortTable(string state_, int order, int info) {
    map<string, int> sorted_data; //uses different map based on input
    map<int, int> sorted_int_data;
    for (int ii = 0; ii < table[getKey(state_, table.size())].size(); ii++) {
        if (table[getKey(state_, table.size())][ii].getState() == state_) { //checks for state in the key since there were some collisions
            if (info == 1) {
